...
class DeepMMTrainer:
    def __init__(self, data_folder_path="data", batch_size=32):
        self.batch_size = batch_size
        self.data = BatchGenerator(data_folder_path, self.batch_size)
    ...
    def train_model(self, epochs=30, log_tensorboard = True, save_models = True):
        ...
        if (log_tensorboard):
            callbacks = [K.callbacks.TensorBoard(log_dir='./models/logs')]
        else:
            callbacks = []

        print("Training the model...")
        self._model.fit_generator(self.data.generator(),
                steps_per_epoch = self.data.m // (self.data.batch_size * self.data.T_x) -1,
                epochs=epochs, 
                validation_freq=1,
